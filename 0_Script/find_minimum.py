...
def catch_mins(monitors_dict, co2_prominence, tvoc_prominence, pm2p5_prominence, temp_prominence, humidity_prominence):
    result = {}
    ...
    for filename, monitor_list in monitors_dict.items():
        ...
        for monitor_df in monitor_list:
            ...
            inverted_signal = -monitor_df[monitor_column]
            
            # Find peaks in the inverted signal
            valleys, _ = find_peaks(inverted_signal, prominence=prominence)
            
            # Convert valley indices to original signal
            valley_times = monitor_df['datetime'].iloc[valleys]
            valley_values = monitor_df[monitor_column].iloc[valleys]
            valley_dict = {time: value for time, value in zip(valley_times, valley_values)}
            
            result[filename].append({monitor_column: valley_dict})
    
    return result



def find_non_overlapping_mins_combined(file_data, window_hours=5):
    all_peaks = {}
...
def process_catch_mins_output(catch_peaks_output, window_hours=5):
    result = {}
    for filename, file_data in catch_peaks_output.items():
        result[filename] = find_non_overlapping_mins_combined(file_data, window_hours)
    return result